...
# New finding_fingerprints first seen since T (relative or ISO)
...
side effect). Real cloud upload is out of scope; `sync --metadata` without `--dry-run` exits
with guidance and opens no sockets.
